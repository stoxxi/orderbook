...
describe("FatalEngineError", () => {
  ...
  describe("Usage Pattern", () => {
    test("should be catchable separately from OrderBookError", () => {
      ...
      const classify = (err: unknown): "fatal" | "book" | "other" => {
        if (err instanceof FatalEngineError) return "fatal";
        // FatalEngineError must NOT be an OrderBookError, so this branch only
        // fires for a genuine OrderBookError — the property under test.
        if (err instanceof OrderBookError) return "book";
        return "other";
      };

      // Both errors must land in their own branch. The previous version of this
      // test only ever threw `fatalError` and asserted the book branch was NOT
      // taken — which was vacuously true and never exercised the `instanceof
      // OrderBookError` path at all.
      expect(classify(fatalError)).toBe("fatal");
      expect(classify(bookError)).toBe("book");
      // And crucially, a FatalEngineError is not misclassified as an OrderBookError.
      expect(fatalError instanceof OrderBookError).toBe(false);
    });
  });
});
